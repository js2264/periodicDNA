...
) 
{
    if (skip_shuffling == FALSE & 'dists_shuffled' %in% names(results)) {
        d <- rbind(
            head(
                cbind(results$hist[-c(seq_len(5)),], type = 'observed'), 100
            ),
            head(
                cbind(
    ...
    p0 <- ggplot2::ggplot(
        head(results$hist[-c(seq_len(5)),], 100), 
...
#' Own ggplot2 theming function, adapted from 
#'     https://github.com/hrbrmstr/hrbrthemes/blob/master/R/roboto-condensed.r
#'
...
theme_ggplot2 <- function(
    ...
    axis_col = "#cccccc", axis = FALSE, ticks = FALSE) {
        ...
        if (border == TRUE) {
            ret <- ret + theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.5))
        }
