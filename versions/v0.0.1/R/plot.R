...
plotSNRs <- function(list_SNRs) {
    require(ggplot2)
    lapply(list_SNRs, function(SNRs) {
        ggplot(SNRs, aes(x = period, y = SNR, color = name)) + 
            geom_point() +
            theme_classic() + 
            labs(
                x = paste0(SNRs$motif, ' period'), 
                y = 'Signal to noise ratio', 
                title = paste0('Strenght of\n', SNRs$motif, ' periodicity'), 
                color = 'Tissue-specific promoters'
            ) + 
            facet_wrap(~name)
    })
}