...
getSignificantPeriods(FPI) %>% head(30)
plotFPI(FPI)
```

**Important:**  
Note that `getFPI()` can also be internally ran when using `getPeriodicity()` 
...
getSignificantPeriods(periodicity_result) %>% head(30)
plotPeriodicityResults(periodicity_result)
```
